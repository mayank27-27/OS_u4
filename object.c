...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // 5. Deduplication: if the object already exists, nothing to do
    if (object_exists(&id)) {
        *id_out = id;
        free(full_obj);
        return 0;
    ...
    // 6. Create shard directory (.pes/objects/XX/) if needed
    char hex[HASH_HEX_SIZE + 1];
    hash_to_hex(&id, hex);
    char shard_dir[512];
    ...
    // 9. fsync the temp file so data reaches disk before rename
    fsync(fd);
    close(fd);
    ...
    // 11. fsync the shard directory to persist the directory entry
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // 6. Extract data (everything after the '\0')
